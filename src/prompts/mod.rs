...
pub trait Prompt<C: Command>: Sized {
    ...
    ///
    /// If the fallback value already set by [`with_fallback()`](`Self::with_fallback`) is rejected
    /// by a new filter, the fallback is silently dropped. This behavior can be changed to a `panic!`
    /// instead of dropping the value by enabling the "no-fallback-drop" feature.
    #[must_use]
